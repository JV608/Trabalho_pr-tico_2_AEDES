...
class Grafo {
  ...
  final int TERRENO_NORMAL = 0; 
  final int TERRENO_LENTO = 1; 
  final int TERRENO_OCUPADO = 2; 
  ...
  void desenhar(ArrayList<Integer> caminho, int destinoIdx) {
    ...
    // Desenha o feedback visual para o terreno lento
    for (int i = 0; i < numVertices; i++) { 
      if (terreno[i] == TERRENO_LENTO) { // 
        PVector pos = posicoes[i]; // 
        noStroke(); // 
        rect(pos.x - l / 2, pos.y - h / 2, l, h); // 
    } 
    }
